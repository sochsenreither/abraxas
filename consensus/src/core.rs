...
pub enum ConsensusMessage {
    ...
    ProposeJolteon(Block),
    VoteJolteon(Vote),
    TimeoutJolteon(Timeout),
    TCJolteon(TC),
    SignedQCJolteon(SignedQC),
    RandomnessShareJolteon(RandomnessShare),
    RandomCoinJolteon(RandomCoin),
    ...
    TimeoutVaba(Timeout),
    TCVaba(TC),
    SignedQCVaba(SignedQC),
    RandomnessShareVaba(RandomnessShare),
    RandomCoinVaba(RandomCoin),
...
impl Core {
    ...
    async fn local_timeout_round(&mut self) -> ConsensusResult<()> {
        ...
        self.timer.reset();
        let message = ConsensusMessage::TimeoutJolteon(timeout.clone());
    ...
    async fn handle_timeout(&mut self, timeout: &Timeout) -> ConsensusResult<()> {
        ...
        if let Some(tc) = self.aggregator.add_timeout(timeout.clone())? {
            ...
            // Broadcast the TC.
            let message = ConsensusMessage::TCJolteon(tc.clone());
            Synchronizer::transmit(
                message,
                &self.name,
                None,
    ...
    async fn handle_sync_request(
        ...
        if let Some(bytes) = self.store.read(digest.to_vec()).await? {
            ...
            let message = ConsensusMessage::SyncReplyJolteon(block);
            Synchronizer::transmit(
                message,
                &self.name,
                Some(&sender),
            ...
            let result = tokio::select! {
                Some(message) = self.core_channel.recv() => {
                    ...
                    match message {
                        ...
                        ConsensusMessage::TimeoutJolteon(timeout) => self.handle_timeout(&timeout).await,
                        ConsensusMessage::TCJolteon(tc) => self.handle_tc(tc).await,
                        ConsensusMessage::SignedQCJolteon(signed_qc) => self.handle_signed_qc(signed_qc).await,
                        ConsensusMessage::RandomnessShareJolteon(rs) => self.handle_rs(rs).await,
                        ConsensusMessage::RandomCoinJolteon(rc) => self.handle_rc(rc).await,
