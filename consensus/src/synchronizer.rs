                ...
                tokio::select! {
                        ...
                        if pending.insert(block.digest()) {
                            ...
                            if !requests.contains_key(&parent){
                                ...
                                let message = match protocol {
                                    SubProto::Jolteon => ConsensusMessage::SyncRequestJolteon(parent, name),
                                    SubProto::Vaba => ConsensusMessage::SyncRequestVaba(parent, name),
                                };
                    ...
                    Some(result) = waiting.next() => match result {
                        Ok(block) => {
                            ...
                            let message = match protocol {
                                    SubProto::Jolteon => ConsensusMessage::LoopBack(block),
                                    SubProto::Vaba => ConsensusMessage::LoopBack(block),
                            };
                    ...
                    () = &mut timer => {
                        ...
                        for (digest, timestamp) in &requests {
                            ...
                            if timestamp + (sync_retry_delay as u128) < now {
                                ...
                                let message = match protocol {
                                    SubProto::Jolteon => ConsensusMessage::SyncRequestJolteon(digest.clone(), name),
                                    SubProto::Vaba => ConsensusMessage::SyncRequestVaba(digest.clone(), name),
                                };
                                Self::transmit(message, &name, None, &network_filter, &committee).await.unwrap();
                            }
                        }
